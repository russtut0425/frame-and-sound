...
function loadEntries(){try{const stored=JSON.parse(localStorage.getItem(STORAGE_KEY));return Array.isArray(stored)&&stored.length?stored:structuredClone(seedEntries);}catch{return structuredClone(seedEntries);}}
...
function render(){
  const films=entries.filter(e=>e.type==="film");const albums=entries.filter(e=>e.type==="album");const average=entries.length?entries.reduce((sum,e)=>sum+Number(e.rating),0)/entries.length:0;
  $("#film-count").textContent=String(films.length).padStart(2,"0");$("#album-count").textContent=String(albums.length).padStart(2,"0");$("#average-rating").textContent=average.toFixed(1);$("#all-tab-count").textContent=entries.length;$("#film-tab-count").textContent=films.length;$("#album-tab-count").textContent=albums.length;
  const normalized=query.trim().toLowerCase();
  const shown=entries.filter(e=>activeFilter==="all"||e.type===activeFilter).filter(e=>!normalized||[e.title,e.subtitle,e.creator,e.tags,e.summary].join(" ").toLowerCase().includes(normalized)).sort((a,b)=>activeSort==="rating"?b.rating-a.rating:b.loggedDate.localeCompare(a.loggedDate));
  $("#entry-grid").innerHTML=shown.length?shown.map((entry,index)=>`<article class="entry-card" data-id="${entry.id}" tabindex="0" style="--accent:${accents[entry.accent]||accents.red}"><div class="card-art" data-index="${String(index+1).padStart(2,"0")}"><div class="art-shape ${entry.type}"><span>${entry.type==="film"?"◐":"◉"}</span></div><span class="type-badge">${entry.type==="film"?"FILM":"ALBUM"}</span>${entry.favorite?'<span class="favorite" aria-label="心爱作品">♥</span>':""}</div><div class="card-body"><div class="card-title-row"><div><h2>${escapeHTML(entry.title)}</h2><p>${escapeHTML(entry.subtitle)}</p></div><strong>${Number(entry.rating).toFixed(1)}</strong></div><p class="creator">${escapeHTML(entry.creator)}${entry.releaseYear?` · ${entry.releaseYear}`:""}</p><blockquote>“${escapeHTML(entry.summary)}”</blockquote><div class="tag-row">${tagsOf(entry).slice(0,3).map(tag=>`<span>#${escapeHTML(tag)}</span>`).join("")}</div></div></article>`).join(""):'<div class="no-results"><span>∅</span><p>没有找到。也可能它还没被你记下来。</p></div>';
}

function openForm(entry=null){
  const form=$("#entry-form");form.reset();form.elements.loggedDate.value=new Date().toISOString().slice(0,10);form.elements.rating.value="8.0";form.elements.type.value="film";form.elements.id.value="";
...
function setType(type){$("#entry-form").elements.type.value=type;$$('[data-type]').forEach(button=>button.classList.toggle("active",button.dataset.type===type));$("#creator-label").textContent=type==="film"?"导演":"音乐人";}
function closeForm(){$("#form-backdrop").classList.add("hidden");}
function openDetail(id){const entry=entries.find(item=>item.id===id);if(!entry)return;selectedId=id;const modal=$("#detail-modal");modal.style.setProperty("--accent",accents[entry.accent]||accents.red);modal.innerHTML=`<button class="close-button" data-close="detail" aria-label="关闭">×</button><span class="type-badge">${entry.type==="film"?"FILM NOTE":"LISTENING NOTE"}</span><div class="detail-score">${Number(entry.rating).toFixed(1)}<small>/ 10</small></div><h2>${escapeHTML(entry.title)}</h2><p class="detail-subtitle">${escapeHTML(entry.subtitle)}</p><p class="creator">${escapeHTML(entry.creator)}${entry.releaseYear?` · ${entry.releaseYear}`:""}</p><blockquote>“${escapeHTML(entry.summary)}”</blockquote><div class="detail-note"><span>留下来的部分</span><p>${escapeHTML(entry.note||"还没写下更多。").replace(/\n/g,"<br>")}</p></div><div class="detail-bottom"><div class="tag-row">${tagsOf(entry).map(tag=>`<span>#${escapeHTML(tag)}</span>`).join("")}</div><time>${escapeHTML(entry.loggedDate)}</time></div><div class="detail-actions"><button data-action="edit">编辑记录</button><button class="danger" data-action="delete">删除这条记录</button></div>`;$("#detail-backdrop").classList.remove("hidden");}
function closeDetail(){$("#detail-backdrop").classList.add("hidden");selectedId=null;}

$("#entry-form").addEventListener("submit",event=>{event.preventDefault();const form=new FormData(event.currentTarget);const title=String(form.get("title")||"").trim();const creator=String(form.get("creator")||"").trim();const rating=Number(form.get("rating"));if(!title||!creator||!Number.isFinite(rating)||rating<0||rating>10){const error=$("#form-error");error.textContent="作品名、作者和 0—10 分评分需要填好。";error.classList.remove("hidden");return;}const id=Number(form.get("id"))||Date.now();const existing=entries.find(item=>item.id===id);const entry={id,type:form.get("type"),title,subtitle:String(form.get("subtitle")||"").trim(),creator,releaseYear:form.get("releaseYear")?Number(form.get("releaseYear")):null,rating,loggedDate:String(form.get("loggedDate")),summary:String(form.get("summary")||"").trim(),note:String(form.get("note")||"").trim(),tags:String(form.get("tags")||"").trim(),favorite:form.get("favorite")==="on",accent:existing?.accent||accentNames[Math.floor(Math.random()*accentNames.length)]};entries=existing?entries.map(item=>item.id===id?entry:item):[entry,...entries];saveEntries();render();closeForm();notify(existing?"改好了":"记下来了");});
