...
function render(){
  ...
  $("#entry-grid").innerHTML=shown.length?shown.map((entry,index)=>{
    ...
    if(entry.type==="album")return `<article class="entry-card album-card" data-id="${escapeHTML(entry.id)}" role="button" aria-label="查看 ${escapeHTML(entry.title)}" tabindex="0" style="--accent:${accents[entry.accent]||accents.red}">
      ...
      <div class="card-body"><div class="album-caption"><span>ALBUM</span>${entry.releaseYear?`<span>${escapeHTML(entry.releaseYear)}</span>`:""}</div>
        <div class="card-title-row"><div><h2>${escapeHTML(entry.title)}</h2>${entry.subtitle?`<p>${escapeHTML(entry.subtitle)}</p>`:""}</div><strong>${ratingLabel(entry)}</strong></div>
    ...
    return `<article class="entry-card ${entry.type==="game"?"game-card":""}" data-id="${escapeHTML(entry.id)}" role="button" aria-label="查看 ${escapeHTML(entry.title)}" tabindex="0" style="--accent:${accents[entry.accent]||accents.red}"><div class="card-art ${image?`has-image ${entry.type}-image`:""}" data-index="${String(index+1).padStart(2,"0")}">${art}<span class="type-badge">${copy.badge}</span>${entry.favorite?'<span class="favorite" aria-label="心爱作品">♥</span>':""}</div><div class="card-body"><div class="card-title-row"><div><h2>${escapeHTML(entry.title)}</h2><p>${escapeHTML(entry.subtitle)}</p></div><strong>${ratingLabel(entry)}</strong></div><p class="creator">${escapeHTML(entry.creator)}${entry.releaseYear?` · ${escapeHTML(entry.releaseYear)}`:""}</p>${gameInfo}<blockquote>“${escapeHTML(entry.summary)}”</blockquote><div class="tag-row">${tagsOf(entry).slice(0,3).map(tag=>`<span>#${escapeHTML(tag)}</span>`).join("")}</div></div></article>`;
