...
function imageOf(entry){
  ...
  const identity=entryIdentity(entry);const automatic=window.filmCoverMap?.[identity]||window.gameCoverMap?.[identity]||window.albumCoverMap?.[identity]||"";
...
function render(){
  ...
  const shown=entries.filter(e=>activeFilter==="all"||e.type===activeFilter).filter(e=>!normalized||[e.title,e.subtitle,e.creator,e.tags,e.summary,e.platform,gameStatuses[e.status]].join(" ").toLowerCase().includes(normalized)).sort((a,b)=>{if(activeSort!=="rating")return b.loggedDate.localeCompare(a.loggedDate);const aRating=ratingOf(a);const bRating=ratingOf(b);if(aRating===null&&bRating===null)return b.loggedDate.localeCompare(a.loggedDate);if(aRating===null)return 1;if(bRating===null)return -1;return bRating-aRating;});
  $("#entry-grid").innerHTML=shown.length?shown.map((entry,index)=>{
    const copy=typeCopy[entry.type]||typeCopy.film;const image=imageOf(entry);const symbol=entry.type==="film"?"◐":entry.type==="album"?"◉":"◆";const art=image?`<img class="entry-image" src="${image}" alt="${escapeHTML(entry.title)}" loading="lazy" decoding="async">`:`<div class="art-shape ${entry.type}"><span>${symbol}</span></div>`;const gameMeta=gameMetaOf(entry);const gameInfo=gameMeta.length?`<div class="game-meta">${gameMeta.map(item=>`<span>${escapeHTML(item)}</span>`).join("")}</div>`:"";
    return `<article class="entry-card ${entry.type==="game"?"game-card":""}" data-id="${entry.id}" tabindex="0" style="--accent:${accents[entry.accent]||accents.red}"><div class="card-art ${image?`has-image ${entry.type}-image`:""}" data-index="${String(index+1).padStart(2,"0")}">${art}<span class="type-badge">${copy.badge}</span>${entry.favorite?'<span class="favorite" aria-label="心爱作品">♥</span>':""}</div><div class="card-body"><div class="card-title-row"><div><h2>${escapeHTML(entry.title)}</h2><p>${escapeHTML(entry.subtitle)}</p></div><strong>${ratingLabel(entry)}</strong></div><p class="creator">${escapeHTML(entry.creator)}${entry.releaseYear?` · ${entry.releaseYear}`:""}</p>${gameInfo}<blockquote>“${escapeHTML(entry.summary)}”</blockquote><div class="tag-row">${tagsOf(entry).slice(0,3).map(tag=>`<span>#${escapeHTML(tag)}</span>`).join("")}</div></div></article>`;
  }).join(""):`<div class="no-results"><span>∅</span><p>${activeFilter==="game"?"还没有游戏记录。第一局还在门外抽烟。":"没有找到。也可能它还没被你记下来。"}</p></div>`;
}

function openForm(entry=null){
  const form=$("#entry-form");form.reset();form.elements.loggedDate.value=new Date().toISOString().slice(0,10);form.elements.rating.value="";form.elements.status.value="playing";form.elements.type.value="film";form.elements.id.value="";pendingImage=typeof entry?.image==="string"?entry.image:"";$("#entry-image").value="";updateImagePreview();
...
function setType(type){
  ...
  if(type==="game"&&!$("#entry-form").elements.status.value)$("#entry-form").elements.status.value="playing";
}
function closeForm(){$("#form-backdrop").classList.add("hidden");}
function openDetail(id){
  const entry=entries.find(item=>item.id===id);if(!entry)return;selectedId=id;const modal=$("#detail-modal");const copy=typeCopy[entry.type]||typeCopy.film;const image=imageOf(entry);const rating=ratingOf(entry);const score=rating===null?'—<small>待评分</small>':`${rating.toFixed(1)}<small>/ 10</small>`;const gameMeta=gameMetaOf(entry);const gameInfo=gameMeta.length?`<div class="detail-game-meta">${gameMeta.map(item=>`<span>${escapeHTML(item)}</span>`).join("")}</div>`:"";modal.style.setProperty("--accent",accents[entry.accent]||accents.red);modal.classList.toggle("game-detail",entry.type==="game");modal.innerHTML=`<button class="close-button" data-close="detail" aria-label="关闭">×</button><span class="type-badge">${copy.detailBadge}</span><div class="detail-score">${score}</div>${image?`<div class="detail-image ${entry.type}"><img src="${image}" alt="${escapeHTML(entry.title)}" decoding="async"></div>`:""}<h2>${escapeHTML(entry.title)}</h2><p class="detail-subtitle">${escapeHTML(entry.subtitle)}</p><p class="creator">${escapeHTML(entry.creator)}${entry.releaseYear?` · ${entry.releaseYear}`:""}</p>${gameInfo}<blockquote>“${escapeHTML(entry.summary)}”</blockquote><div class="detail-note"><span>${copy.detailNote}</span><p>${escapeHTML(entry.note||"还没写下更多。").replace(/\n/g,"<br>")}</p></div><div class="detail-bottom"><div class="tag-row">${tagsOf(entry).map(tag=>`<span>#${escapeHTML(tag)}</span>`).join("")}</div><time>${escapeHTML(entry.loggedDate)}</time></div><div class="detail-actions"><button data-action="edit">编辑记录</button><button class="danger" data-action="delete">删除这条记录</button></div>`;$("#detail-backdrop").classList.remove("hidden");
}
function closeDetail(){$("#detail-backdrop").classList.add("hidden");selectedId=null;}

$("#entry-form").addEventListener("submit",async event=>{event.preventDefault();const form=new FormData(event.currentTarget);const type=String(form.get("type")||"film");const title=String(form.get("title")||"").trim();const creator=String(form.get("creator")||"").trim();const ratingText=String(form.get("rating")||"").trim();const rating=ratingText===""?null:Number(ratingText);const hoursText=String(form.get("hours")||"").trim();const hours=type==="game"&&hoursText!==""?Number(hoursText):null;if(!title||!creator||(rating!==null&&(!Number.isFinite(rating)||rating<0||rating>10))||(hours!==null&&(!Number.isFinite(hours)||hours<0))){const error=$("#form-error");error.textContent="作品名和创作者需要填好；评分可以留空，填写时需在 0—10 分之间。";error.classList.remove("hidden");return;}const id=Number(form.get("id"))||Date.now();const existing=entries.find(item=>item.id===id);const previousEntries=entries;const entry={id,type,title,subtitle:String(form.get("subtitle")||"").trim(),creator,releaseYear:form.get("releaseYear")?Number(form.get("releaseYear")):null,rating,loggedDate:String(form.get("loggedDate")),platform:type==="game"?String(form.get("platform")||"").trim():"",status:type==="game"?String(form.get("status")||"playing"):"",hours,summary:String(form.get("summary")||"").trim(),note:String(form.get("note")||"").trim(),tags:String(form.get("tags")||"").trim(),favorite:form.get("favorite")==="on",accent:existing?.accent||accentNames[Math.floor(Math.random()*accentNames.length)],image:pendingImage};const button=$("#save-button");button.disabled=true;button.textContent="正在保存…";try{await storeEntryImage(entry,existing?.image||"");entries=existing?entries.map(item=>item.id===id?entry:item):[entry,...entries];if(!saveEntries()){entries=previousEntries;return;}render();closeForm();notify(existing?"改好了":"记下来了");}catch{entries=previousEntries;notify("图片保存失败，再试一次");}finally{button.disabled=false;button.textContent=existing?"保存修改":"收进档案";}});
