...
function imageOf(entry){return typeof entry?.image==="string"&&/^data:image\/(?:jpeg|png|webp);base64,/i.test(entry.image)?entry.image:"";}
...
function render(){
  ...
  $("#entry-grid").innerHTML=shown.length?shown.map((entry,index)=>{
    const image=imageOf(entry);const art=image?`<img class="entry-image" src="${image}" alt="${escapeHTML(entry.title)}">`:`<div class="art-shape ${entry.type}"><span>${entry.type==="film"?"◐":"◉"}</span></div>`;
...
function openForm(entry=null){
  const form=$("#entry-form");form.reset();form.elements.loggedDate.value=new Date().toISOString().slice(0,10);form.elements.rating.value="8.0";form.elements.type.value="film";form.elements.id.value="";pendingImage=imageOf(entry||{});$("#entry-image").value="";updateImagePreview();
...
function openDetail(id){const entry=entries.find(item=>item.id===id);if(!entry)return;selectedId=id;const modal=$("#detail-modal");const image=imageOf(entry);modal.style.setProperty("--accent",accents[entry.accent]||accents.red);modal.innerHTML=`<button class="close-button" data-close="detail" aria-label="关闭">×</button><span class="type-badge">${entry.type==="film"?"FILM NOTE":"LISTENING NOTE"}</span><div class="detail-score">${Number(entry.rating).toFixed(1)}<small>/ 10</small></div>${image?`<div class="detail-image ${entry.type}"><img src="${image}" alt="${escapeHTML(entry.title)}"></div>`:""}<h2>${escapeHTML(entry.title)}</h2><p class="detail-subtitle">${escapeHTML(entry.subtitle)}</p><p class="creator">${escapeHTML(entry.creator)}${entry.releaseYear?` · ${entry.releaseYear}`:""}</p><blockquote>“${escapeHTML(entry.summary)}”</blockquote><div class="detail-note"><span>留下来的部分</span><p>${escapeHTML(entry.note||"还没写下更多。").replace(/\n/g,"<br>")}</p></div><div class="detail-bottom"><div class="tag-row">${tagsOf(entry).map(tag=>`<span>#${escapeHTML(tag)}</span>`).join("")}</div><time>${escapeHTML(entry.loggedDate)}</time></div><div class="detail-actions"><button data-action="edit">编辑记录</button><button class="danger" data-action="delete">删除这条记录</button></div>`;$("#detail-backdrop").classList.remove("hidden");}
